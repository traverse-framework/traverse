...
and `1259-portable-authority-contracts`.
**Decision evidence**: Traverse #1384; Decision 84; ADR-0071.
...
uses the same request/result/event/error envelopes.
...
- **FR-010 — Opaque references**: Success payload MAY include `artifact_ref`
  as a host-managed opaque string. It MUST NOT include raw audio, file
  paths, URLs, device identifiers, provider names, or credentials.
...
- **FR-013 — Same port**: `model.execute` on `traverse.model-runtime` MUST
  use the same command/result/event/error types as `audio.capture`.
  Provider, model id, endpoint, and credential fields are forbidden.
...
7. `model.execute` succeeds through the same port with a distinct
   activated `traverse.model-runtime` binding.
