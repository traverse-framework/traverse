...
Traverse is pre-1.0 (runtime/crates v0.11.0). "Runs everywhere" is the design
...
|---|---|---|
| **Native** | Shipped | `traverse-runtime`: `NativeExecutor`, `ThreadPoolExecutor`, Wasmtime `WasmExecutor`; nested-wasmi path via app-owned `runtime.wasm`. |
| **Browser** | Shipped | `traverse-embedder-web` on npm (currently **0.11.0**; lockstep with the runtime line). Bundles must include `runtime/runtime.wasm` (+ `.sha256`). |
| **Rust app embedding** | Shipped | `traverse-embedder` **0.11.0** on crates.io; CI `embedder_conformance/rust_package.sh`. |
