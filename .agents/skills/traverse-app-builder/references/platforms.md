...
Traverse is pre-1.0 (runtime/crates v0.10.1). "Runs everywhere" is the design
...
| **Browser** | Shipped | `traverse-embedder-web` on npm (currently **0.9.0**; runtime line is 0.10.1). |
| **Rust app embedding** | Shipped | `traverse-embedder` **0.10.1** on crates.io; CI `embedder_conformance/rust_package.sh`. |
